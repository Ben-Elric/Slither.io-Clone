...
public:
	...
	// 刷新数据
	void FlashData(short& n, int& dx, int& dy)
	{
		if (n == frame)
		{
...
int main()
{
	...
	Game game;//运行
	game.Running();
	PlaySound( L"E:\\！学习课程\C&C++（class）\\程序设计\\蛇蛇大作战\\Agar.io Snake beta\\res\\Sound\\WAV\\陆行鸟.wav", NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
	settextcolor(WHITE);
